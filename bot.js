...
import fs from 'fs';
import { findMinimumTime, divideObject, createStationsKeyboard } from './functions.js';
import { registerUser, setLine, getLine } from './db.js';
...
bot.addListener('message', async (msg) => {
    ...
    if (['احسان به دستغیب', 'دستغیب به احسان'].includes(msg.text)) {
        setLine(msg.chat.id.toString(), msg.text).then(() => {
    ...
    if (userLine) {
        await bot.sendMessage(msg.chat.id, userLine + '\n' + 'مبدا را انتخاب کنید', {
            reply_markup: {
        ...
        await bot.sendMessage(msg.chat.id, 'لطفا خط مورد نظر را انتخاب کنید', {
            reply_markup: {
                keyboard: [
                    [{ text: 'احسان به دستغیب' }], [{ text: 'دستغیب به احسان' }]
                ],
                resize_keyboard: true,
...
bot.addListener('callback_query', async (callbackQuery) => {
    ...
    const buttonLabels = JSON.parse(fs.readFileSync('origins.json', 'utf8')).Shiraz;
